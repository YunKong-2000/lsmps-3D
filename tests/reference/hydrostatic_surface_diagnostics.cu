...
int main(int argc, char** argv) {
  ...
  const lsmps3d::NeighborSearchConfig neighbor_config{
      lsmps3d::CellGrid{
          lsmps3d::Vec3{-kSupportRadius, -kSupportRadius, -kSupportRadius},
          kSupportRadius,
          lsmps3d::Int3{grid_dim, grid_dim, grid_dim},
      },
      kSupportRadius,
  };
  ...
  const lsmps3d::SurfaceDetectionConfig surface_config{
      ...
      12,
      static_cast<lsmps3d::real>(0.85),
      static_cast<lsmps3d::real>(0.33),
      static_cast<lsmps3d::real>(0.05),
      true,
  ...
  const lsmps3d::LegacyVtkWriter vtk_writer({output_dir, "hydrostatic_surface", true});
